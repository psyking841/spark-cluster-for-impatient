...
if [ -z "$status" ] || [ "$status" -eq "Stopped" ]; then
    ...
    minikube start --memory=8192 --cpus=4 --vm-driver=hyperkit --kubernetes-version=v1.14.1 --disk-size=30g
fi

if ! type helm &> /dev/null ; then
    curl https://storage.googleapis.com/kubernetes-helm/helm-v2.13.1-darwin-amd64.tar.gz -o /tmp/helm-v2.13.1-darwin-amd64.tar.gz
    tar xvzf /tmp/helm-v2.13.1-darwin-amd64.tar.gz -C /tmp/
    /tmp/darwin-amd64/helm init
    /tmp/darwin-amd64/helm install $(dirname "$0")/../charts/spark-cluster
else
    helm init
...
kubectl exec -it $SPARK_MASTER -- pyspark --driver-cores 1 --driver-memory 2Gi --num-executors 2 --total-executor-cores 2 --executor-memory 2Gi
